...
Connecter.cursor.execute("SELECT * FROM acctester")
for row in Connecter.cursor.fetchall():
    print(row)
    print(row[0])
    print(type(row[0]))
...
#curser.fetchone()
#("a",101)
Connecter.database.commit()
Connecter.cursor.close()
Connecter.database.close()
